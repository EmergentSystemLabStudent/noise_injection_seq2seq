...
verbs = ['go', 'move', 'take', 'get', 'detect', 'find', 'grasp', 'bring', 'leave',
         'come', 'exit', 'call', 'carry', 'place', 'put', 'throw', 'ask', 'follow']
        ...
        if verb in ('go', 'move'):
            ...
            try:
                if sentence_verb[0] in ('take', 'get', 'detect', 'find', 'grasp'):
                    ...
                    if verb == 'take':
                        if sentence_verb == []:
                            item = check_command_items(
                                sentence_non_verb.pop(0))
                            ...
                            item = check_command_items(
                                sentence_non_verb.pop(0))
                            if verb == 'bring':
                                place2 = check_command_places(
                                    sentence_non_verb.pop(0)[2:])
                                command_verb.append('Find')
                                command_objective.append((item, place))
                                command_verb.append('Grasp')
                                command_objective.append(item)
                        ...
                        if sentence_verb == []:
                            ...
                            if verb == 'bring':
                                ...
                                command_verb.append('Move')
                                command_objective.append(
                                    check_command_places(sentence_non_verb.pop(0)))
                                command_verb.append('Grasp')
                            elif verb == 'follow':
                                command_verb.append('Find')
                                command_objective.append(('person', place))
...
def assemble_command(command_verb, command_objective):
    ...
    for (verb, objective) in zip(command_verb, command_objective):
        if len(objective) == 2:
            assembled_command = assembled_command + verb + \
                ' ( ' + objective[0] + ' ' + objective[1] + ' ) '
        else:
            assembled_command = assembled_command + \
                verb + ' ( ' + objective + ' ) '
