    ...
    if(str(labels) != "nan"):
        for label in labels.split(' '):
            ...
            if label.find(u'ROOM') > -1:
                ...
                label = label.replace(
                    u'LOCATION', locationLabel[locationLabelCounter])
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(15000)):
        ...
        sentence = pd.DataFrame([fillIn(labelSetdatas[0])], columns=[
                                "data", "labeldata"])
        df = df.append(sentence)
    df.drop_duplicates(["data"])
    df.to_csv("tempGPSRSentence.csv", index=False,
              mode='w', header=False, encoding="cp932")
