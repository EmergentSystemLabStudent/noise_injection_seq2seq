...
def SpeechRecognition(filename):
    ...
    units = {'0': 'zero', '1': 'one', '2': 'two', '3': 'three', '4': 'four',
    ...
    result = [filename, google_result["alternative"][0]['transcript'], " ".join(
        google_phoneme), sphinx_result.hyp().hypstr, " ".join(sphinx_phoneme)]
    return result
...
if __name__ == '__main__':
    ...
    df1 = pd.DataFrame(results, columns=[
                       "filename", "google_word", "google_phoneme", "sphinx_word", "sphinx_phoneme"])
    df2 = pd.read_csv("GPSRsentence_list.csv")
    df = pd.merge(df1, df2, left_index=True, right_index=True)
    df.to_csv(out_dir+"SpeechRecognitionResults.csv", columns=[
              "filename", "original_sentence", "google_word", "google_phoneme", "sphinx_word", "sphinx_phoneme"])
